...
Shared STRAFE 64 menu canvas: a black MAGI terminal with a drifting
green grid, CRT scanlines and a containment frame. Drawn behind every
fullscreen menu so the whole UI — not just the main screen — reads as
one NERV system.
=================
*/
void UI_DrawNervBackdrop( void ) {
	int		i;
	float	off;
	vec4_t	voidcol = { 0.020f, 0.016f, 0.035f, 1.00f };
	vec4_t	grid    = { 0.10f,  0.55f,  0.22f, 0.14f };
	vec4_t	scan    = { 0.00f,  0.00f,  0.00f, 0.16f };
	vec4_t	frame   = { 0.10f,  0.55f,  0.22f, 0.30f };

	UI_FillRect( 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, voidcol );

	off = (float)( ( uis.realtime / 40 ) % 32 );
	for ( i = -32; i <= SCREEN_WIDTH; i += 32 ) {
		UI_FillRect( i + off, 0, 1, SCREEN_HEIGHT, grid );
	}
	for ( i = 0; i <= SCREEN_HEIGHT; i += 32 ) {
		UI_FillRect( 0, i, SCREEN_WIDTH, 1, grid );
	}
	for ( i = 0; i < SCREEN_HEIGHT; i += 3 ) {
		UI_FillRect( 0, i, SCREEN_WIDTH, 1, scan );
	}
	UI_DrawRect( 8, 8, SCREEN_WIDTH - 16, SCREEN_HEIGHT - 16, frame );
}
...
void UI_Refresh( int realtime )
{
	...
	if ( uis.activemenu )
	{
		...
		{
			// STRAFE 64: one NERV canvas behind every fullscreen menu,
			// replacing OpenArena's gothic background art
			UI_DrawNervBackdrop();
		}
