...
//	savedAreaBits = client->areabits;
	...
	raceBestTime = client->raceBestTime;	// best run survives death; the run itself doesn't
...
//	client->areabits = savedAreaBits;
	...
	client->raceBestTime = raceBestTime;
	...
	// STRAFE 64: loadout-shop weapon purchases persist across death — you
	// bought it, it's yours for the session. Armor/heal stay per-life
	// consumables; only the weapon unlocks carry. Re-grant with a little ammo.
	if ( client->boughtWeapons ) {
		client->ps.stats[STAT_WEAPONS] |= client->boughtWeapons;
		for ( i = 0 ; i < WP_NUM_WEAPONS ; i++ ) {
			if ( ( client->boughtWeapons & ( 1 << i ) ) && client->ps.ammo[i] <= 0 ) {
				client->ps.ammo[i] = 20;
			}
		}
	}
	if ( client->boughtAirJump ) {
