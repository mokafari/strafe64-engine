...
struct gclient_s {
	...
	int			idleSeconds;		// hot floor: seconds spent near-still on the ground

	int			raceStartTime;		// level.time the race start pad was last touched, 0 = not racing
	int			raceBestTime;		// best finish in ms this session, 0 = none
	int			raceLaps;			// completed laps this session (core-loop currency counter)
...
typedef struct {
	...
	// rising void (from worldspawn "voidbase"/"voidrise"/"voiddelay")
	qboolean	voidActive;
...
extern	vmCvar_t	g_speedDamage;
extern	vmCvar_t	g_speedDamageMax;
extern	vmCvar_t	g_selfDamageScale;
extern	vmCvar_t	g_killReward;
extern	vmCvar_t	g_ammoRegen;
extern	vmCvar_t	g_hotFloor;
...
extern	vmCvar_t	g_timeBindMin;
extern	vmCvar_t	g_timeBindMax;
...
extern	vmCvar_t	g_airStopAccel;
extern	vmCvar_t	g_airControl;
extern	vmCvar_t	g_playtest;			// bot playtest telemetry on/off
extern	vmCvar_t	g_playtestTag;		// output file tag: playtest/<tag>.jsonl
extern	vmCvar_t	g_playtestTrail;	// also log 100ms position trails (heatmaps/tracers)
