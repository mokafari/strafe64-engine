...
void player_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath ) {
	...
	// bot playtest telemetry: record this bot's run as a death, and the
	// frag on the attacker's run (combat metrics for the ARENA dojo)
	G_PlaytestDeath( self, obit );
	G_PlaytestKill( attacker, self );
	...
	if (attacker && attacker->client) {
		...
		if ( attacker == self || OnSameTeam (self, attacker ) ) {
			...
			// fragging fuels the run: health and armor on every kill
			if ( g_killReward.integer ) {
				attacker->health += 25;
				...
				attacker->client->ps.stats[STAT_ARMOR] += 10;
				if ( attacker->client->ps.stats[STAT_ARMOR] > attacker->client->ps.stats[STAT_MAX_HEALTH] * 2 ) {
					attacker->client->ps.stats[STAT_ARMOR] = attacker->client->ps.stats[STAT_MAX_HEALTH] * 2;
				}
			}
...
speed = damage: the curve shared by damage, knockback, and the HUD.
1.0 at run speed, +50% per extra run speed of velocity, capped at
g_speedDamageMax; near-standstill bottoms out at 0.85
============
*/
static float G_SpeedDamageScale( gentity_t *attacker ) {
	float	speed, scale, baseline;

	if ( !g_speedDamage.integer || !attacker || !attacker->client ) {
		return 1.0f;
	}
	...
	scale = speed / baseline;
	if ( scale < 1.0f ) {
		scale = 0.85f + 0.15f * scale;
	} else {
		scale = 1.0f + ( scale - 1.0f ) * 0.5f;
		if ( scale > g_speedDamageMax.value ) {
			scale = g_speedDamageMax.value;
		}
	}
	return scale;
}
...
void G_Damage( gentity_t *targ, gentity_t *inflictor, gentity_t *attacker,
	...
	knockback = damage;
	// fast attackers launch people: same curve as speed damage
	// (self-knockback stays unscaled so rocket jumps are consistent)
	if ( g_speedKnockback.integer && targ != attacker ) {
		knockback = knockback * G_SpeedDamageScale( attacker );
	}
	if ( knockback > 280 ) {
		knockback = 280;
	}
	...
	// self damage is cheap, calculated after knockback so rocket
	// jumping costs little health but keeps its full kick
	if ( targ == attacker ) {
		damage *= g_selfDamageScale.value;
	}
	// speed = damage: the attacker's velocity scales what they deal
	else {
		damage *= G_SpeedDamageScale( attacker );
	}
