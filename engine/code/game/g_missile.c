...
void G_MissileImpact( gentity_t *ent, trace_t *trace ) {
	...
	if (other->takedamage) {
		...
		if ( ent->damage ) {
			...
			impactDamage = ent->damage;

			// midair: direct rockets on airborne players hit 1.5x
			if ( ent->s.weapon == WP_ROCKET_LAUNCHER && other->client
				&& other->client->ps.groundEntityNum == ENTITYNUM_NONE
				&& ent->r.ownerNum != other->s.number ) {
				impactDamage = impactDamage * 3 / 2;
				if ( g_entities[ent->r.ownerNum].client ) {
					trap_SendServerCommand( ent->r.ownerNum, "cp \"MIDAIR!\"" );
				}
			}

			G_Damage (other, ent, &g_entities[ent->r.ownerNum], velocity,
				ent->s.origin, impactDamage,
				0, ent->methodOfDeath);
		}
	}
...
STRAFE 64: a projectile carries a fraction of its owner's horizontal
momentum, so a fast strafe sweeps your shots into an arc with you — lead
with your body. Vertical aim is left honest; re-snapped for the network.
=================
*/
static void G_InheritVelocity( gentity_t *self, vec3_t trDelta, float frac ) {
	if ( !self->client || frac <= 0.0f ) {
		return;
	}
...
gentity_t *fire_plasma (gentity_t *self, vec3_t start, vec3_t dir) {
	...
	G_InheritVelocity( self, bolt->s.pos.trDelta, 0.8f );	// Wake: bolts ride your strafe
...
gentity_t *fire_grenade (gentity_t *self, vec3_t start, vec3_t dir) {
	...
	SnapVector( bolt->s.pos.trDelta );			// save net bandwidth
	G_InheritVelocity( self, bolt->s.pos.trDelta, 1.0f );	// Skipper: speed = throw range
...
gentity_t *fire_bfg (gentity_t *self, vec3_t start, vec3_t dir) {
	...
	VectorScale( dir, 2000, bolt->s.pos.trDelta );
	SnapVector( bolt->s.pos.trDelta );			// save net bandwidth
	G_InheritVelocity( self, bolt->s.pos.trDelta, 0.4f );	// Overdrive
	VectorCopy (start, bolt->r.currentOrigin);
...
gentity_t *fire_rocket (gentity_t *self, vec3_t start, vec3_t dir) {
	...
	SnapVector( bolt->s.pos.trDelta );			// save net bandwidth
	G_InheritVelocity( self, bolt->s.pos.trDelta, 0.35f );	// Lance: stays aimable
	VectorCopy (start, bolt->r.currentOrigin);
