...
vmCvar_t	g_speedDamage;
vmCvar_t	g_speedDamageMax;
vmCvar_t	g_selfDamageScale;
vmCvar_t	g_killReward;
vmCvar_t	g_ammoRegen;
vmCvar_t	g_hotFloor;
...
vmCvar_t	g_airStopAccel;		// -> pm_airStopAccelerate
vmCvar_t	g_airControl;		// -> pm_airControlAmount (W-only redirect strength)
vmCvar_t	g_playtest;
vmCvar_t	g_playtestTag;
vmCvar_t	g_playtestTrail;
vmCvar_t	g_password;
vmCvar_t	g_needpass;
...
static cvarTable_t		gameCvarTable[] = {
	...
	// movement mod combat cvars
	{ &g_speedDamage, "g_speedDamage", "1", CVAR_ARCHIVE, 0, qtrue  },
	{ &g_speedDamageMax, "g_speedDamageMax", "2.0", CVAR_ARCHIVE, 0, qtrue  },
	{ &g_selfDamageScale, "g_selfDamageScale", "0.25", CVAR_ARCHIVE, 0, qtrue  },
	{ &g_killReward, "g_killReward", "1", CVAR_ARCHIVE, 0, qtrue  },
	{ &g_ammoRegen, "g_ammoRegen", "1", CVAR_ARCHIVE, 0, qtrue  },
	{ &g_hotFloor, "g_hotFloor", "1", CVAR_ARCHIVE, 0, qtrue  },
	...
	{ &g_airStopAccel, "pm_airStopAccelerate", "3", CVAR_ARCHIVE, 0, qfalse  },
	{ &g_airControl, "pm_airControlAmount", "150", CVAR_ARCHIVE, 0, qfalse  },
	{ &g_playtest, "g_playtest", "0", 0, 0, qfalse  },
	{ &g_playtestTag, "g_playtestTag", "", 0, 0, qfalse  },
	{ &g_playtestTrail, "g_playtestTrail", "0", 0, 0, qfalse  },
...
void G_InitGame( int levelTime, int randomSeed, int restart ) {
	...
	// STRAFE 64 daily mutator rotation: g_mutator 9 = "daily" — resolve it to a
	// concrete twist (1..3) by the calendar day and write it back, so the whole
	// pipeline (per-frame effect, spawn announce, serverinfo->HUD) sees a real
	// value. Same day -> same twist for everyone, fresh each day. Ties the run
	// mutators into the daily-speedrun identity.
	if ( g_mutator.integer == 9 ) {
		qtime_t	now;
		int		daily;
		trap_RealTime( &now );
...
void G_ShutdownGame( int restart ) {
	...
	// flush any bots still mid-run to the playtest log
	G_PlaytestFlush();
...
The rising void: a kill plane that climbs from below the course on a
pace schedule set by the map's worldspawn (voidbase / voidrise /
voiddelay, written by strafegen). Fall behind the pace and the world
catches you.
================
*/
void G_RunVoid( void ) {
	...
	for ( i = 0 ; i < level.maxclients ; i++ ) {
		...
		if ( ent->r.currentOrigin[2] < voidZ ) {
			// the void doesn't wound — it erases
			G_Damage( ent, NULL, NULL, NULL, NULL,
				1000, DAMAGE_NO_PROTECTION, MOD_FALLING );
		}
	}
}
