...
STRAFE 64 loadout shop — the core-loop spend. Laps bank PERS_SCORE in
race_finish_touch; here you spend it to extend your kit, CS-buy-bind style:
"buy" alone lists the shop + your balance, "buy <item>" purchases if you can
afford it and don't already own it. Kit rides through the lap teleport (which
doesn't respawn you), so each lap you come back richer and better-armed —
the WC3-mod-for-CS progression loop.
=================
*/
typedef struct {
	const char	*name;
	int			cost;
	int			weapon;		// WP_* to grant, or WP_NONE
	int			ammo;		// rounds added with the weapon
	int			armor;		// armor added (capped 200)
	int			heal;		// 1 = top up to max health
	int			airjump;	// 1 = grant the permanent +1 air jump (movement kit)
} shopItem_t;

static const shopItem_t g_shop[] = {
	{ "rocket",  400,  WP_ROCKET_LAUNCHER, 20, 0,   0, 0 },
	{ "rail",    700,  WP_RAILGUN,         15, 0,   0, 0 },
	{ "bfg",     1200, WP_BFG,             10, 0,   0, 0 },
	{ "armor",   300,  WP_NONE,             0, 100, 0, 0 },
	{ "heal",    200,  WP_NONE,             0, 0,   1, 0 },
	{ "airjump", 800,  WP_NONE,             0, 0,   0, 1 },
};
#define G_SHOP_COUNT ((int)(sizeof(g_shop)/sizeof(g_shop[0])))

void Cmd_StrafeBuy_f( gentity_t *ent ) {
	char				arg[MAX_TOKEN_CHARS];
	int					i, score, a;
	const shopItem_t	*s;
	...
	if ( !arg[0] ) {
		trap_SendServerCommand( ent - g_entities,
			va( "print \"^3LOADOUT SHOP^7  (score %i)\n\"", score ) );
		for ( i = 0; i < G_SHOP_COUNT; i++ ) {
			trap_SendServerCommand( ent - g_entities, va(
				"print \"  buy %-7s %5i\n\"", g_shop[i].name, g_shop[i].cost ) );
		}
		return;
	}
	...
	s = NULL;
	for ( i = 0; i < G_SHOP_COUNT; i++ ) {
		if ( Q_stricmp( arg, g_shop[i].name ) == 0 ) {
			s = &g_shop[i];
			break;
		}
	}
	if ( !s ) {
		trap_SendServerCommand( ent - g_entities,
			va( "print \"no such item: %s  (type buy)\n\"", arg ) );
		return;
	}
	if ( ( s->weapon != WP_NONE
			&& ( ent->client->ps.stats[STAT_WEAPONS] & ( 1 << s->weapon ) ) )
		|| ( s->airjump && ent->client->boughtAirJump ) ) {
		trap_SendServerCommand( ent - g_entities,
			va( "cp \"ALREADY OWN %s\"", s->name ) );
		return;
	}
	if ( score < s->cost ) {
		trap_SendServerCommand( ent - g_entities,
			va( "cp \"NEED %i  (have %i)\"", s->cost, score ) );
		return;
	}

	AddScore( ent, ent->r.currentOrigin, -s->cost );		// pay
	if ( s->weapon != WP_NONE ) {
		ent->client->ps.stats[STAT_WEAPONS] |= ( 1 << s->weapon );
		ent->client->ps.ammo[s->weapon] += s->ammo;
		ent->client->boughtWeapons |= ( 1 << s->weapon );	// persists across death
	}
	if ( s->armor ) {
		a = ent->client->ps.stats[STAT_ARMOR] + s->armor;
		if ( a > 200 ) {
			a = 200;
		}
		ent->client->ps.stats[STAT_ARMOR] = a;
	}
	if ( s->heal ) {
		ent->health = ent->client->ps.stats[STAT_MAX_HEALTH];
	}
	if ( s->airjump ) {
		ent->client->boughtAirJump = 1;					// persists across death
		ent->client->ps.pm_flags |= PMF_AIRJUMP_BONUS;
	}
	trap_SendServerCommand( ent - g_entities, va(
		"cp \"BOUGHT %s  -%i  (%i LEFT)\"",
		s->name, s->cost, ent->client->ps.persistant[PERS_SCORE] ) );
}
