...
void CG_ParseServerinfo( void ) {
	...
	cgs.lattice = atoi( Info_ValueForKey( info, "g_lattice" ) );
	...
	cgs.vectorgun = atoi( Info_ValueForKey( info, "g_vectorgun" ) );
	cgs.mutator = atoi( Info_ValueForKey( info, "g_mutator" ) );
}
...
CS_VOIDINFO is "<baseZ> <rise ups/s> <startTimeMs>"; empty means the
map has no rising void. The client integrates the plane height locally
so nothing needs per-frame updates.
================
*/
void CG_ParseVoidInfo( void ) {
	const char	*info;

	info = CG_ConfigString( CS_VOIDINFO );
	if ( !info[0] ) {
		cgs.voidActive = qfalse;
		return;
	}
	cgs.voidBase = atof( COM_Parse( (char **)&info ) );
	cgs.voidRise = atof( COM_Parse( (char **)&info ) );
	cgs.voidStartTime = atoi( COM_Parse( (char **)&info ) );
	cgs.voidActive = ( cgs.voidRise > 0 );
}
