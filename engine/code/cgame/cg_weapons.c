...
void CG_RegisterWeapon( int weaponNum ) {
	...
	switch ( weaponNum ) {
	case WP_GAUNTLET:	// Slipstream — cyan-white
		MAKERGB( weaponInfo->flashDlightColor, 0.5f, 1.0f, 1.0f );
	...
	case WP_MACHINEGUN:	// Tracer — pale yellow
		MAKERGB( weaponInfo->flashDlightColor, 1.0f, 0.95f, 0.5f );
	...
	case WP_SHOTGUN:	// Choke — orange
		MAKERGB( weaponInfo->flashDlightColor, 1.0f, 0.55f, 0.15f );
	...
	case WP_ROCKET_LAUNCHER:
		...
		MAKERGB( weaponInfo->missileDlightColor, 1.0f, 0.45f, 0.1f );	// Lance — amber-red
		MAKERGB( weaponInfo->flashDlightColor, 1.0f, 0.45f, 0.1f );
	...
	case WP_GRENADE_LAUNCHER:
		...
		weaponInfo->wiTrailTime = 700;
		weaponInfo->trailRadius = 32;
		weaponInfo->missileDlight = 150;	// Skipper — glowing green bomb
		MAKERGB( weaponInfo->missileDlightColor, 0.4f, 1.0f, 0.35f );
		MAKERGB( weaponInfo->flashDlightColor, 0.4f, 1.0f, 0.35f );
...
//		weaponInfo->missileModel = cgs.media.invulnerabilityPowerupModel;
		...
		MAKERGB( weaponInfo->flashDlightColor, 1.0f, 0.2f, 1.0f );	// Wake — magenta
	...
	case WP_RAILGUN:
		...
		MAKERGB( weaponInfo->flashDlightColor, 0.2f, 0.7f, 1.0f );	// Vectorgun — void blue
	...
	case WP_BFG:
		...
		MAKERGB( weaponInfo->flashDlightColor, 0.5f, 1.0f, 0.6f );	// Overdrive — green-white
		weaponInfo->flashSound[0] = trap_S_RegisterSound( "sound/weapons/bfg/bfg_fire.wav", qfalse );
		cgs.media.bfgExplosionShader = trap_R_RegisterShader( "bfgExplosion" );
		weaponInfo->missileModel = trap_R_RegisterModel( "models/weaphits/bfg.md3" );
		weaponInfo->missileDlight = 200;
		MAKERGB( weaponInfo->missileDlightColor, 0.5f, 1.0f, 0.6f );
		weaponInfo->missileSound = trap_S_RegisterSound( "sound/weapons/rocket/rockfly.wav", qfalse );
		break;
...
static void CG_SwordSlashAnglesForQuads( int startQuad, int endQuad, float f, qboolean finisher, vec3_t out );
...
void CG_DrawWeaponSelect( void ) {
	...
	{
		// tint the whole weapon bar NERV amber, keeping the fade alpha
		vec4_t	bar;
		bar[0] = 1.0f; bar[1] = 0.6f; bar[2] = 0.06f; bar[3] = color[3];
		trap_R_SetColor( bar );
	}
	...
	if ( cg_weapons[ cg.weaponSelect ].item ) {
		...
		if ( name ) {
			...
			nc[0] = 1.0f; nc[1] = 0.6f; nc[2] = 0.06f; nc[3] = color[3];
			w = CG_MatrixStringWidth( name, 2.2f );
			x = ( SCREEN_WIDTH - w ) / 2;
			CG_DrawMatrixString( x, y - 28, name, 2.2f, nc );
		}
	}
