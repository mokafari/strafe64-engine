...
static void CG_RegisterSounds( void ) {
	...
	cgs.media.flowSound = trap_S_RegisterSound( "sound/world/telein.wav", qfalse );
	// speed milestone announcer; normally only registered in team games
	cgs.media.holyShitSound = trap_S_RegisterSound( "sound/feedback/voc_holyshit.wav", qtrue );
...
void CG_Init( int serverMessageNum, int serverCommandSequence, int clientNum ) {
	...
	// per-map persistent speed record: register an archived cvar keyed by
	// the map name, so each map keeps its own all-time peak-speed PB (a
	// fast map's record can't make a tight map's record unbeatable)
	...
	// load the best ghost run for this map
	CG_GhostInit();
