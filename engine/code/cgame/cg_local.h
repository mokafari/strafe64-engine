...
typedef struct {
	...
	// movement mod: session top-speed milestone tier (speed / 100)
	int			speedTier;

	// movement mod: hysteretic flow-state latch (drives the entry surge)
	qboolean	inFlow;
	int			flowSurgeTime;	// instant of the last flow-entry crossing
	int			peakSpeed;		// top horizontal speed this life (speedrun PB)
	...
	int			hitMarkerTime;
	qboolean	hitMarkerFast;	// landed while moving fast enough to be lethal

	// FLOW combo: style multiplier + run score (cgame-side scoring & juice)
	float		comboMult;			// live style multiplier, 1..COMBO_MAX
	float		styleScore;			// style points banked this run
	int			styleBest;			// best style score this session
	int			lifeStylePeak;		// peak styleScore this life (cleared on respawn) — the run's style for the report
	int			comboEventTime;		// cg.time of the last combo-feeding trick
	int			comboMilestone;		// highest whole-x milestone announced
	int			comboBhopSeen;		// edge trackers for trick detection
	int			comboWalljumpSeen;
	int			comboAirjumpSeen;
	int			comboScoreSeen;		// PERS_SCORE, to catch frags
	int			comboRaceSeen;		// STAT_RACE_START, to reset on a new run
	int			comboLandTime;		// cg.landTime edge, for the landing punch
	float		comboDamageSeen;	// cg.damageTime edge, for the hit shake
	int			trickTime;			// cg.time the trick callout was set
	char		trickName[24];		// e.g. "WALL RUN", "MIDAIR FRAG"
	float		viewShake;			// screen-shake magnitude (deg), decays
	float		fovPunch;			// transient FOV addition (deg), decays
	...
	// movement mod: momentary directional damage indicator
	int			dmgDirTime;
	float		dmgDirAngle;	// bearing to the attacker, radians from view forward

	// movement mod: amber pop when a frag heals you (kill reward)
	int			fragFlashTime;
...
typedef struct {
	...
	sfxHandle_t hopSoundHigh;	// streak 5+
	sfxHandle_t hopSoundMax;	// streak 9+
	sfxHandle_t flowSound;		// surge on crossing into flow speed
...
typedef struct {
	...
	int				vectorgun;			// g_vectorgun from serverinfo (mirrored into pmove)
	int				mutator;			// g_mutator from serverinfo (0 none, 1 low-g, 2 rush, 3 heavy)

	// rising void, from CS_VOIDINFO
	qboolean		voidActive;
	float			voidBase;			// z the void starts at
	float			voidRise;			// units per second
	int				voidStartTime;		// server time the rise begins
