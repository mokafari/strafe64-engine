...
static void S_OpenBackgroundStream( const char *filename ) {
	...
	// the stream is resampled from info.rate to dma.speed and mixed as
	// stereo, so any rate works; only truly broken formats are worth a warning
	if(s_backgroundStream->info.channels < 1 || s_backgroundStream->info.channels > 2
		|| s_backgroundStream->info.rate < 8000) {
		Com_Printf(S_COLOR_YELLOW "WARNING: music file %s has an unusual format\n", filename );
	}
}
